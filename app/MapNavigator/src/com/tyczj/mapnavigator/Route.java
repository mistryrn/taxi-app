	...
	private void parseRoute(JSONObject obj3){
		try{
			...
			if(!obj3.isNull("distance")){
				...
				totalDistance = Double.parseDouble(obj.getString("value"));
			}
			
			if(!obj3.isNull("duration")){
				JSONObject obj = obj3.getJSONObject("duration");
				totalDuration = Double.parseDouble(obj.getString("value"));
			}
			...
			for(Legs leg : legs){
				for(Steps step : leg.getSteps()){
					...
					totalDuration = totalDuration + leg.getLegDuration();
				}	
			}
