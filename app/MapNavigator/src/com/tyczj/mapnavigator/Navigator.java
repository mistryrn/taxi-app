...
public class Navigator {
	...
	public Navigator(GoogleMap map, LatLng startLocation, LatLng endLocation, String from){
	...
	public ArrayList<Double> getDistance(){
		//Log.d("Navigator2", ""+distance + " " +from);
		return distance;
	}
		...
		protected void onPostExecute(Directions directions){
			...
			if(directions != null){
				...
				for(int i=0; i<directions.getRoutes().size(); i++){
					...
					Navigator.this.distance.add(r.getDistance());
					Navigator.this.duration.add(r.getDuration());
					//Log.d("Navigator", distance.get(i).toString() + " " + context + " " +from);
					Log.d("Navigator", ""+distance);
					if(i == 0){
